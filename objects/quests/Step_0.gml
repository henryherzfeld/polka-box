...
var i = 0; repeat(quests_grid_n) {
		...
		switch(i) {
			...
			case quest_pb.tutorial: {
				...
				switch(step) {
					...
					case 4: 
						if update {
							scr_progress_quest(quest_pb.hints) // CLICKING ITEM TO USE IT
					...
					case 5: 
						if update {
							...
							scr_progress_quest(quest_pb.hints);  // PRESSING F TO USE ITEM
					...
					case 7:
						if update {
							scr_progress_quest(quest_pb.hints);  // CLICKING NOTEBOOK ICON TO VIEW EVIDENCE
					...
					case 8: 				
						if update {
							...
							scr_progress_quest(quest_pb.hints);  // HOVER ITEM TO SEE, CLICK EXIT TO LEAVE
							var inst = instance_create_layer(100, 100, "Meta", obj_cutscene);
							inst.scene_info = [	
					...
					case 12: {
						...
						if inst == noone {
							scr_progress_quest(i);
							scr_audio_play_sound(snd_segment_complete, 1, false);
						}
			...
			case quest_pb.erosion_investigation:
				switch(step) {
					...
					case 6: {
						...
						if polka.itemEquiped == enum_item_type.camera and inst != noone and polka.input_use_item {
							scr_audio_play_sound(snd_clue_item, 1, false);
					...
					case 17: 
							if update { scr_audio_play_sound(snd_segment_complete, 1, false); }
					...
					case 20: 
						...
						if update {
							...
							scr_progress_quest(quest_pb.erosion_experiment);
							scr_progress_quest(i);
						}
						break;
			...
			case quest_pb.erosion_experiment:
				save = false;
				switch(step) {
					...
					case 3: 
						...
						if update {
							scr_save_game();
							scr_progress_quest(quest_pb.erosion_case);
							scr_evi_add_notebook(enum_evi_type.soil_experiment_tbl, true);
							scr_audio_play_sound(snd_segment_complete, 1, false); // sound effect
						}
					...
					case 4:
						...
						if room != rm_weeraway_interior and room != rm_weeraway_interior_dinner_1 and
						   room != rm_weeraway_interior_dinner_2 and room != erosion_village and room != rm_empty {
							scr_audio_play_sound(snd_quest_complete, 1, false);
		...
		switch(i) {
			...
			case quest_mtv.polka: {
				switch(step) {
					...
					case 3: {
						if room == second_village_planting and obj_planting_minigame.is_done  { 
							scr_progress_quest(i);
							scr_audio_play_sound(snd_segment_complete, 0, false);
						}
					...
					case 4: {
						if room == rm_poppy {
							// create and run poppy cutscene
							var inst = instance_create_layer(0, 0, "Meta", obj_cutscene);
							inst.scene_info = scr_poppy_minigame_1();
							inst.active = true;
					...
					case 5:
						if room == second_village {
							...
							inst.do_animation = true;
							scr_audio_play_sound(snd_quest_complete, 0, false);
							scr_progress_quest(i);
						}
						break;
					case 6:
						ev = event.talk_poppy;
						if room == second_village {
							if instance_find(obj_npc_poppy, 0) == noone {
								var inst = instance_create_layer(520, 1167, "Characters", obj_npc_poppy);
								inst.watering = true;
								scr_char_change_dialogue(obj_npc_poppy, 2);
								scr_char_update_dialogue(obj_npc_poppy);
								
								scr_event_register_script(event.talk_poppy, obj_npc_poppy, scr_char_change_dialogue_randrange, obj_npc_poppy, 3, 5);
								scr_event_register_script(event.talk_poppy, obj_npc_poppy, scr_char_update_dialogue, obj_npc_poppy);
								
							}
						
						} else if room == rm_poppy {
							instance_destroy(obj_npc_poppy);
						}
						break;
					case 7:
						if room == second_village {
							if instance_find(obj_npc_poppy, 0) == noone {
								if not instance_find(obj_npc_poppy, 0) {
									var inst = instance_create_layer(525, 1167, "Characters", obj_npc_poppy);
									inst.watering = true;
								}
								scr_char_change_dialogue(obj_npc_poppy, irandom_range(3, 5));
								scr_char_update_dialogue(obj_npc_poppy);
	...
	if update and step < objectives_n and step >= 0 {
		...
		if objectives[step] != noone { 
			...
			if game.mode == "pb" and i != quest_pb.hints {
