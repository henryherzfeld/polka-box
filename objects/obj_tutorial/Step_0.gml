...
switch (tut_step){
	...
	case 3: {
		if(scr_check_objective(enum_objective_type.ero_poppy0) and !polka.in_dialogue){
				if(!counter){
					text = ds_map_create();
	...
	case 4: {
			if(scr_check_objective(enum_objective_type.ero_examine_crops0) and !polka.in_dialogue){
				if(!counter){
					text = ds_map_create();
	...
	case 5: {
			if(scr_check_objective(enum_objective_type.ero_poppy1) and !polka.in_dialogue){
				if(!counter){
					text = ds_map_create();
