function scr_dialogue_baron() {
	...
	scr_dia_line("Why, thank you! I got her from the planet Euboxia.", obj_npc_baron);
	scr_dia_options("A1", "A2", "A3", "EXIT");
	...
	*/
	...
	["The key fell down the toilet", "An animal might have taken the key", "You swallowed the key", "The key was dissolved by strong acid"], 
	1);
	scr_quiz_question_response("How did you find out?", 
	"Are you sure that's what happened?");
	scr_quiz_question_sprite(spr_give_conclusion); 
	...
	scr_quiz_question_sprite(spr_lost_heart_info); 
	...
	*/
	scr_quiz_question_response("Is that so?", 
	"You almost got it! Try again");
	scr_quiz_question_sprite(spr_evidence_select_info); 
	//END OF BEGINNING TUTORIAL QUIZ QUESTIONS

	scr_dia_segment(text4, "GREET");
	scr_dia_line("Is that so?", obj_npc_baron);
	scr_dia_line_quiz(beginning_tutorial_quiz_questions, obj_npc_baron);

	text5 = ds_map_create();
	scr_dia_segment(text5, "GREET");
	scr_dia_line("Aha, excellent work!", obj_npc_baron);
	scr_dia_line("You just finished your first Quest investigation!", obj_npc_baron);
	scr_dia_line("I'm confident that you'll do great out there!", obj_npc_baron);

	scr_dia_line("You'll find these tiles throughout the world.", obj_npc_baron); 
	scr_dia_line_sprite(spr_coin_tiles); 

	scr_dia_line("Go to them to win some coins!", obj_npc_baron); 

	scr_dia_line("You can use these coins to buy SKINS and ITEMS from the shop.", obj_npc_baron); 
	...
	*/
	...
	scr_dia_line("To solve the problem, you will help the scientists with doing an EXPERIMENT.", obj_npc_baron);


	scr_dia_line("Experimental Research is a skill that's helpful for ANYONE.", obj_npc_baron);
	scr_dia_line_sprite(spr_tutorial_entrepreneur); 

	scr_dia_line("Not just scientists and researchers.", obj_npc_baron);
	scr_dia_line("Even some entrepreneurs can do it.", obj_npc_baron);
	scr_dia_line("An entrepreneur is somebody who runs a business and sells things.", obj_npc_baron);


	scr_dia_line("Experimental Research is all about testing something you want to find out if it's true.", obj_npc_baron);
	scr_dia_line("Say your friend runs a lemonade stand.", obj_npc_baron);
	scr_dia_line_sprite(spr_tutorial_entrepreneur);
	scr_dia_line_sprite(spr_lemonade_stand);

	scr_dia_line("He wants to add honey instead of sugar to lemonade because he thinks it'll make the lemonade taste better.", obj_npc_baron);
	...
	scr_dia_line("You'd want to know where he got the idea from.", obj_npc_baron);
	scr_dia_line_sprite(spr_magazine_article_honey); 

	scr_dia_line("He tells you an article he read said that adding honey instead of sugar to lemonade makes it taste better.", obj_npc_baron);
	scr_dia_line("This is his BACKGROUND RESEARCH.", obj_npc_baron);


	scr_dia_line("But what if people don't like honey?", obj_npc_baron);

	scr_dia_line("What if his customers think that lemonade tastes good with sugar and not with honey?", obj_npc_baron);
	scr_dia_line_sprite(spr_magazine_article_honey); 

	scr_dia_line("Then it means your friend wasted a lot of money.", obj_npc_baron);
	scr_dia_line("And you'll trust your friend less with your money.", obj_npc_baron);
	...
	2);
	scr_quiz_question_response("So your friend sets up an EXPERIMENT to try this out with a little honey before he goes out and buys a whole bunch of honey.", "You almost got it! Try again");
	...
	scr_quiz_question_response("Your friend makes a HYPOTHESIS to let you know what exactly he's trying to find out.", "You almost got it! Try again");

	scr_quiz_question(enum_question_type.multi, 
	"His hypothesis is that...", 
	["Inviting bears to the stand will scare people from coming", 
	"Adding honey instead of sugar to lemonade will make people like it more", 
	"Squeezing lemon juice into apple juice will make for a better drink", 
	"Adding sugar instead of honey to lemonade will make people like it more"], 
	1);
	scr_quiz_question_sprite(spr_hypothesis_question_marks); 
	scr_quiz_question_response("Then you'd ask...", "You almost got it! Try again");
	...
	scr_quiz_question_response("Your friend sets up a PROCEDURE for what he'll be doing during the experiment.", "You almost got it! Try again");

	//END OF EROSION EXPERIMENT TUTORIAL 2 QUIZ QUESTIONS

	scr_dia_line("Your friend sets up a PROCEDURE for what he'll be doing during the experiment.", obj_npc_baron);
	scr_dia_line_quiz(erosion_experiment_tutorial_2, obj_npc_baron);

	scr_dia_line("He has TWO GROUPS.", obj_npc_baron);
	scr_dia_line_sprite(spr_experiment_tutorial_setup);

	scr_dia_line("He will give half of his customers lemonade with sugar, but without honey.", obj_npc_baron);

	scr_dia_line("This is his FIRST group.", obj_npc_baron);


	scr_dia_line("He will give the other half of his customers the lemonade with honey, but without sugar.", obj_npc_baron);

	scr_dia_line("This is his SECOND group.", obj_npc_baron);


	scr_dia_line("He asks each customer to give a rating, from 1-10, on how much they liked his lemonade.", obj_npc_baron);

	scr_dia_line("Then he adds up the ratings from each group.", obj_npc_baron);
	...
	"Do bears like honey?"], 
	1);
	scr_quiz_question_response("You will want to know what DATA your friend found. Was the honey a success?", "You almost got it! Try again");

	//END OF EROSION EXPERIMENT TUTORIAL 3 QUIZ QUESTIONS

	scr_dia_line("You will want to know what DATA your friend found. Was the honey a success?", obj_npc_baron);
	scr_dia_line_quiz(erosion_experiment_tutorial_3, obj_npc_baron);

	scr_dia_line("Or should he stick with plain sugar?", obj_npc_baron);
	scr_dia_line("He does an analysis of the ratings from each group.", obj_npc_baron);

	scr_dia_line("His ANALYSIS found that the total sum of ratings from the first group was 25.", obj_npc_baron);
	scr_dia_line("The total sum of ratings from the second group was 45.", obj_npc_baron);
	...
	scr_quiz_question_response("Your friend comes up with a CONCLUSION: adding honey instead of sugar to lemonade makes customers like it more.", "You almost got it! Try again");

	//END OF EROSION EXPERIMENT TUTORIAL 4 QUIZ QUESTIONS

	scr_dia_line("Your friend comes up with a CONCLUSION: adding honey instead of sugar to lemonade makes customers like it more.", obj_npc_baron);
	...
	scr_dia_line("Now that your friend has shown you the evidence, you can now trust him more.", obj_npc_baron);
	scr_dia_line("You give him the money needed to buy more honey.", obj_npc_baron);

	scr_dia_line("And that's all there is to an EXPERIMENT!", obj_npc_baron);
	scr_dia_line("Now let's see you with the real deal!", obj_npc_baron);
